    ...
    def __post_init__(self) -> None:
        ...
        if min_needed > self.height:
            ...
            self.row_spacing = max(
                4, min(self.row_spacing, available - 2 * 20)
            )  # min 4px between rows
...
class ControlBar:
    ...
    def __init__(
        self,
        width: int = 800,
        config: ControlBarConfig | None = None,
    ...
    def handle_mouse_move(self, px: int, py: int) -> Button | None:
    ...
    def _get_clicked_button(self, px: int, py: int) -> Button | None:
    ...
    def get_button(self, action_name: str) -> Button | None:

...
class ReferenceSpectrumLoader:
    ...
    def load_blackbody(self) -> Optional[ReferenceSpectrum]:
        """Load blackbody 6500K spectrum."""
        path = self.data_dir / "blackbody_6500K.csv"
        return self._load_two_column_csv(
            path, 
            "Blackbody 6500K",
            "Theoretical blackbody radiator at 6500K"
        )
    
    def load_solar(self) -> Optional[ReferenceSpectrum]:
    ...
    def load_mercury_lamp(self) -> Optional[ReferenceSpectrum]:
    ...
    def load_fluorescent(self) -> Optional[ReferenceSpectrum]:
    ...
    def load_d65_daylight(self) -> Optional[ReferenceSpectrum]:
    ...
    def _load_two_column_csv(
        self, 
        path: Path, 
        name: str, 
        source: str
    ) -> Optional[ReferenceSpectrum]:
...
class ReferenceSpectrumManager:
    ...
    @property
    def current(self) -> Optional[ReferenceSpectrum]:
